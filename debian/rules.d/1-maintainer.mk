...
printchanges:
	@baseCommit=$$(git log --pretty=format:'%H %s' | \
		gawk '/UBUNTU: '".*Ubuntu-lts-`echo $(prev_fullver)~14.04.1 | sed 's/+/\\\\+/'`"'$$/ { print $$1; exit }'); \
...
startnewrelease:
	dh_testdir
	@nextminor=$(shell expr `echo $(revision) | awk -F~ '{print $$1}' | awk -F. '{print $$2}'` + 1); \
	nextmajor=$(shell expr `echo $(revision) | awk -F. '{print $$1}'` + 1); \
	now="$(shell date -R)"; \
	echo "Creating new changelog set for $(release)-$$nextmajor.$$nextminor~14.04.1..."; \
	echo -e "$(src_pkg_name) ($(release)-$$nextmajor.$$nextminor~14.04.1) UNRELEASED; urgency=low\n" > $(DEBIAN)/changelog.new; \
