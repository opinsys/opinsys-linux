 ...
 *	 undertested.
 */
static u8 mt7662u_tx_pktid_enc(struct mt7662u_dev *dev, u8 rate, bool is_probe)
{
...
static void
mt7662u_tx_pktid_dec(struct mt7662u_dev *dev, struct mt76_tx_status *stat)
{
...
static void mt7662u_tx_skb_remove_dma_overhead(struct sk_buff *skb,
...
void mt7662u_tx_status(struct mt7662u_dev *dev, struct sk_buff *skb)
{
	...
	mt7662u_tx_skb_remove_dma_overhead(skb, info);
...
static int mt7662u_skb_rooms(struct mt7662u_dev *dev, struct sk_buff *skb)
{
...
mt7662u_push_txwi(struct mt7662u_dev *dev, struct sk_buff *skb,
		  ...
		  int pkt_len)
{
	...
	pkt_id = mt7662u_tx_pktid_enc(dev, rate_ctl & 0x7, is_probe);
		...
		struct sk_buff *skb)
{
	...
	if (mt7662u_skb_rooms(dev, skb) || mt76_insert_hdr_pad(skb)) {
	...
	txwi = mt7662u_push_txwi(dev, skb, sta, wcid, pkt_len);

	if (mt7662u_dma_enqueue_tx(dev, skb, wcid, hw_q))
		return;
...
void mt7662u_tx_stat(struct work_struct *work)
{
	...
	while (!test_bit(MT7662U_STATE_REMOVED, &dev->state)) {
		stat = mt7662u_mac_fetch_tx_status(dev);
		if (!stat.valid)
			break;

		mt7662u_tx_pktid_dec(dev, &stat);
	...
	if (cleaned)
		queue_delayed_work(dev->stat_wq, &dev->stat_work,
				   msecs_to_jiffies(10));
	else if (test_and_clear_bit(MT7662U_STATE_MORE_STATS, &dev->state))
		queue_delayed_work(dev->stat_wq, &dev->stat_work,
				   msecs_to_jiffies(20));
	else
		clear_bit(MT7662U_STATE_READING_STATS, &dev->state);
		    ...
		    u16 queue, const struct ieee80211_tx_queue_params *params)
{
	 ...
	 *	 See what mt7662u_set_default_edca() used to do in init.c.
	 */
