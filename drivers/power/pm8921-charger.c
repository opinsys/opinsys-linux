...
static int get_prop_batt_ccc(struct pm8921_chg_chip *chip)
{
	int rc;

	rc = pm8921_bms_get_ccc();
	if (rc < 0)
		pr_err("unable to get batt ccc rc = %d\n", rc);
	return rc;
}

static int get_prop_batt_ecc(struct pm8921_chg_chip *chip)
{
	int rc;

	rc = pm8921_bms_get_ecc();
	if (rc < 0)
		pr_err("unable to get batt ccc rc = %d\n", rc);
	return rc;
}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_EMPTY:
		val->intval = get_prop_batt_ecc(chip);
		break;
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		val->intval = get_prop_batt_ccc(chip);
		break;
	default:
		return -EINVAL;
	}
